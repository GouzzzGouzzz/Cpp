...
{
	std::cout << "Character copy constructor called (Deleting current inventory before copying from the copy object)\n";
...
{
	std::cout << "Character copy assignement operator called (Deleting current inventory before copying from the copy object)\n";
	this->_Name = copy._Name;
	for (int i = 0; i < 4; i++)
		if (this->_inventory[i] != NULL)
			delete this->_inventory[i];
